...
server.get('/channels/:id', async (req: FastifyRequest<{ Params: { id: string } }>, res) => {
  ...
  if (cached && cached instanceof VoiceChannel) {
    if (!cached.permissionsFor(cached.guild.id).has(PermissionFlagsBits.ViewChannel)) return res.status(400).send({ code: 'private_channel' });
  ...
  if (!channel.permissionsFor(channel.guild.id).has(PermissionFlagsBits.ViewChannel)) return res.status(400).send({ code: 'private_channel' });
